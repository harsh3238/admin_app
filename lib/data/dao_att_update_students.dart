...
class DaoUpdateAttendance {
  static final DaoUpdateAttendance _singleton = new DaoUpdateAttendance._internal();
  static const String _TB_NAME = "attendance_student_updates";

  Database _db;

  factory DaoUpdateAttendance() {
    return _singleton;
  }

  DaoUpdateAttendance._internal();
  ...
  Future<List<Map<String, dynamic>>> searchStudent(int classId, int sectionId, String searchQuery) async {
    await _init();

    List<Map<String, dynamic>> rs = await _db.query(
      _TB_NAME,
      where: "class_id = ? AND section_id = ? AND student_name LIKE '%$searchQuery%'",
      whereArgs: [classId, sectionId],
    );
    if (rs.length > 0) {
      return rs;
  ...
  /*Future<bool> leaveValidations(int classId, int sectionId) async {
    ...
    int countOfEntriesWhereLeaveIdIsNull = leaveNotSelectedRs[0]["COUNT(s_r_no)"] ?? 0;
  ...
  Future<void> setAbsentReason(int stucareId, String absentReason) async {
    await _init();
    await _db.update(_TB_NAME, {'absent_reason': absentReason},
  ...
  Future<void> deleteAll(int classId, int sectionId) async {
    await _init();
    await _db.delete(_TB_NAME, where: "class_id = ? AND section_id = ?",
        whereArgs: [classId, sectionId]);
    return null;
  }
}
