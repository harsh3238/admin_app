...
class FeeDayWise extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return StateFeeDayWise();
  }
}

class StateFeeDayWise extends State<FeeDayWise> with StateHelper {
  ...
  List<Map<String, String>> contentData = [
    {"stu_fname": "Student Name : "},
    {"s_r_no": "S.R.No. : "},
    {"class_name": "Class : "},
    {"term": "Term : "},
    {"paid_amt": "Amount : "},
    {"paying_date": "Date Paid : "},
  ...
  void _getData() async {
    ...
    if (_selectedResidence != null) {
      requestBody.putIfAbsent('residence_type_id', () => _selectedResidence["id"].toString());
    }

    if ( _selectedClass != null) {
      requestBody.putIfAbsent('class_id', () => _selectedClass["id"].toString());

    }

    if (_selectedUser != null) {
      requestBody.putIfAbsent('user_id', () => _selectedUser["id"].toString());
    }

    modulesResponse =
        await http.post(GConstants.getDayWiseFeesRoute(await AppData().getSchoolUrl()), body: requestBody);
    ...
    if (apiResponse.statusCode == 200) {
      ...
      if (jsonObject.containsKey("success")) {
        if (jsonObject["success"] == true) {
          ...
          _classList = jsonObject['classes'];
          _residenceList = jsonObject['residence'];
          _getData();
          return null;
        } else {
          return null;
        ...
        actions: <Widget>[
          GestureDetector(
            ...
            onTap: () {
              showDialog(
                context: context,
                builder: (BuildContext context) => DayWiseFilter(
                  _userList, _classList, _residenceList
                ),
              ).then((onValue) {
                if(onValue!=null){
                  _selectedUser = onValue[0];
                  _selectedClass = onValue[1];
                  _selectedResidence = onValue[2];
                  _getData();
                }
              });
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: _InputDropdown(
                      ...
                      valueStyle: Theme.of(context).textTheme.subhead.apply(color: Colors.white),
                      onPressed: () async {
                        DateTime firstDate = DateTime.now().subtract(Duration(minutes: 10));
                                ...
                                Container(
                                  child: Row(
                                    ...
                                    children: <Widget>[
                                      Padding(
                                        padding: EdgeInsets.all(8),
                                        child: Text(_duesData[index]["rec_no_label"]!=null?
                                          "Receipt No.: "+_duesData[index]["rec_no_label"].toString():"Receipt No.:  Not Available",
                                          style:
                    ...
                    Container(
                      child: Row(
                        ...
                        children: <Widget>[
                          Padding(
                            padding: EdgeInsets.all(8),
                            child: Text("Total Amount: "+getTotalAmount(),
                              style:
  ...
  String getTotalAmount() {
    ...
    for(final item in _duesData){
      ...
      //
      double amount = double.parse(item["paid_amt"]);
      if(amount!=null){
