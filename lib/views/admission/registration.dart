...
class RegistrationTab extends StatefulWidget {
  StateRegistrationTab _stateRegistrationTab;

  @override
  State<StatefulWidget> createState() {
    _stateRegistrationTab = StateRegistrationTab();
    return _stateRegistrationTab;
  }

  void sessionChanged(TheSession session) {
    _stateRegistrationTab._getData(session.sessionId.toString(),);
  }
}

class StateRegistrationTab extends State<RegistrationTab> with StateHelper {
  ...
  void _getData(sessionId) async {
    ...
    var modulesResponse = await http.post(
        GConstants.getRegistrationRoute(await AppData().getSchoolUrl()),
        body: {
    ...
    return TableRow(
      children: <Widget>[
        Container(
          ...
          child: Text(
            (item == null) ? "Receipt" : item['registration_rec_no'],
            style: (item == null)
        ...
        Container(
          ...
          color: (item == null) ? Colors.white : Colors.transparent,
          child: Text(
            (item == null) ? "Date" : DateFormat().addPattern("dd-MMM").format(DateTime.parse(item['registration_date'])),
            style: (item == null)
                ? TextStyle(fontWeight: FontWeight.bold, color: Colors.indigo)
                : TextStyle(
                    fontWeight: FontWeight.normal,
