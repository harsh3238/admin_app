...
class FormSaleTab extends StatefulWidget {
  StateFormSaleTab _stateFormSaleTab;

  @override
  State<StatefulWidget> createState() {
    _stateFormSaleTab = StateFormSaleTab();
    return _stateFormSaleTab;
  }

  void sessionChanged(TheSession session) {
    _stateFormSaleTab._getData(session.sessionId.toString(),);
  }
}

class StateFormSaleTab extends State<FormSaleTab> with StateHelper {
  ...
  void _getData(sessionId) async {
    ...
    var modulesResponse = await http.post(
        GConstants.getFormSaleRoute(await AppData().getSchoolUrl()),
        body: {
    ...
    return TableRow(
      children: <Widget>[
        Container(
          ...
          child: Text(
            (item == null) ? "Receipt" : item['form_rec_no'],
            style: (item == null)
        ...
        Container(
          ...
          color: (item == null) ? Colors.white : Colors.transparent,
          child: Text(
            (item == null) ? "Date" : DateFormat().addPattern("dd-MMM").format(DateTime.parse(item['form_sale_date'])),
            style: (item == null)
                ? TextStyle(fontWeight: FontWeight.bold, color: Colors.indigo)
                : TextStyle(
                    fontWeight: FontWeight.normal,
